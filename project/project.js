  ...
  if (dataTables) {
    ...
    tableData.forEach((item, index) => {
      ...
      editButton.addEventListener("click", (event) => {
        ...
        // console.log(customer);
        const url = `../BELAJAR/form-edit/form-edit.html?customer=${encodeURIComponent(
          customer
        )}&idOrder=${encodeURIComponent(idOrder)}&dp=${encodeURIComponent(
          dp
        )}&desc=${encodeURIComponent(desc)}&dateIn=${encodeURIComponent(
          dateIn
        )}&dateOut=${encodeURIComponent(dateOut)}&status=${encodeURIComponent(
          status
        )}`;
